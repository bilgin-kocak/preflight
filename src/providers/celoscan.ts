...
export class CeloscanProvider implements Provider {
  ...
  async inspect(address:string,cutoffBlock:number):Promise<Observation> {
    const get=(start=0)=>Promise.all(['txlist','tokentx','txlistinternal'].map(action=>history('https://api.etherscan.io/v2/api',address,action,{chainid:'42220',apikey:this.key},fetch,start)));
  ...
  async health() {try {await history('https://api.etherscan.io/v2/api','0x0000000000000000000000000000000000000000','txlist',{chainid:'42220',apikey:this.key});return true;} catch{return false;} }
}
