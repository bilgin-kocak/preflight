...
The free preview is live at https://preflight-production-9071.up.railway.app. On 2026-09-09, ERC-8004 agent **9826** was registered and the hackathon draft was saved for `askbots-growth` and `judges-favorite`. Payments remain disabled; the first AskBots round is funded and active, with zero completed reviews at activation. No paid API settlements have occurred. No secrets belong in this file. Signed reports, batch, contract checks, tags endpoint, attestation contract, CLI and MCP are deferred per section 9 (Days 2–4).
...
- Created the agent's facilitator API key through an off-chain ownership signature and stored it privately; configured Railway's `X402_API_KEY`. The service remains payment-disabled pending paid activation. The account has 20 mainnet and 1000 testnet settlement credits. Source: https://x402.celo.org/api/account?address=0x7a1f16230c5F3fD3b50C435fa93035eE789eE789 . Current key-creation message and API shape verified in https://x402.celo.org/assets/index-DVsQ-8FQ.js .
...
- Existing project `k17ck4pafdpv9zts8svazajt6s8e3ncq` is **active**, funded for **10 reviews / 1.10 USDT**, with `paidCount=0` and `responsesReceived=0` at 17:03 UTC. Sources: https://www.askbots.ai/api/projects/k17ck4pafdpv9zts8svazajt6s8e3ncq/fund ; authenticated official CLI `projects:myProjects` query at https://calculating-mockingbird-764.convex.cloud/api/query .
...
- Current balance after funding: **3.85 USDT**, **4.936866 USDC**. Total funding gas cost **0.007927 USDC**. Second round remains unfunded. Source: https://forno.celo.org (ERC-20 balanceOf and transaction receipts).
