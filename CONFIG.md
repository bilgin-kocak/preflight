...
The free preview is live at https://preflight-production-9071.up.railway.app. On 2026-09-09, ERC-8004 agent **9826** was registered and the hackathon draft was saved for `askbots-growth` and `judges-favorite`. Payments remain disabled; no paid reviews or settlements have occurred. No secrets belong in this file. Signed reports, batch, contract checks, tags endpoint, attestation contract, CLI and MCP are deferred per section 9 (Days 2–4).
...
- The initial wallet check was zero for CELO, USDC and USDT. The user subsequently funded USDC; see the confirmed post-registration balance above. AskBots still needs separate USDT funding. Source: https://forno.celo.org (eth_getBalance and ERC-20 balanceOf).
...
- Confirmed ABI: `createProject(bytes32 projectId,uint256 budget)` pulls `budget * (costPerResponse + platformFeePerResponse)` USDT from its caller. Read-only mainnet calls confirm USDT address, `costPerResponse=100000`, `platformFeePerResponse=10000`, and `paused=false`; 10 reviews require exactly 1,100,000 atomic USDT (1.10 USDT). No approval, deposit, or activation has been sent. A compliant sender and authenticated AskBots project ID are still needed.
