...
- Live event slug **agents-at-work**. Counting starts **2026-08-28 00:00 UTC**; the extended deadline is **Monday 2026-09-21 09:00 UTC (12:00 Istanbul)** for all tracks. Rechecked September 16; this supersedes the original September 14 schedule. Round two is September 19–20; winners are scheduled for September 25 at 13:00 UTC. Source: https://celobuilders.xyz/hackathons/agents-at-work/timeline .
- Primary track slug **askbots-growth**. **Both rounds require at least 10 reviews**. The improvement gap uses the standard overall question and only counts when round two reaches **5.0/10**. Additional track **judges-favorite** rationale from spec. Source: https://celobuilders.xyz/hackathons/agents-at-work/tracks .
- Current independent-counterparty rules require token movement within the 60 days before August 28 (June 29–August 28), as well as the control/funding exclusions. Day 1's `active_before_cutoff` checks earlier observed activity, not this entire eligibility rule; do not equate it with qualification. Source: https://celobuilders.xyz/hackathons/agents-at-work/rules .
...
- **Registration conflict resolved by user-authorized bootstrap:** the form required an existing identity before issuing its tag. The first ERC-8004 transaction used the SDK-derived Preflight tag `celo_5ffb0641430f` and USDC gas. This bootstrap code is not credited by the hackathon. After identity creation, the authenticated draft save returned official tag `celo_80fe04c6accd`; all future self-sent writes must use that assigned tag. No transaction can be retagged retroactively.
...
- Required skill https://www.askbots.ai/skill.md and docs https://www.askbots.ai/docs fetched/read. The fetched documentation named the bare API host, but the user's migration notice supersedes those examples: send reviewer credentials only to **https://www.askbots.ai/api/**. Do not follow cross-host redirects with credentials; see the integration checks below.
...
The free preview and paid x402 API are live at https://preflight-production-9071.up.railway.app. On 2026-09-09, ERC-8004 agent **9826** was registered and the hackathon draft was saved for `askbots-growth` and `judges-favorite`. The first AskBots round completed all ten reviews. One controlled 0.005 USDC API purchase settled, its SQLite receipt was verified, and replay returned HTTP 409. Two external pilot invitations are sent; independent testing is still pending. No secrets belong in this file. Signed reports, batch, contract checks, tags endpoint, attestation contract, CLI and MCP are deferred per section 9 (Days 2–4). Sources: [baseline](docs/hackathon/baseline.md), [activation evidence](docs/hackathon/evidence/mainnet-payment-smoke.json), [settlement](https://celo.blockscout.com/tx/0x1d8fde59197391d59615cad0ab9d7e460dd605d836bb72c43e49675d7cfe6e1c), [outreach](docs/hackathon/distribution.md).

- `ERC8004_AGENT_ID=9826`; `erc8004Url=https://8004scan.io/agents/celo/9826`. Registry `ownerOf(9826)` confirms `0x7a1f16230c5F3fD3b50C435fa93035eE789eE789`; `tokenURI(9826)` is the live `/.well-known/agent.json` URL. Sources: https://forno.celo.org ; https://celo.blockscout.com/tx/0x4cac2be5ced21315025027f6d09762a7f4fa63c595eaaf8449046b6a7a746c8d .
- `ATTRIBUTION_TAG=celo_80fe04c6accd`, confirmed by authenticated draft save and subsequent GET. Draft ID `426158aa-1643-48f1-b21a-855b9478363c`; final publication pending. Source: https://celobuilders.xyz/submissions/me .
- Identity transaction succeeded in block **77070483**. `verifyTx` decoded bootstrap tag `celo_5ffb0641430f`; actual transaction feeCurrency equals the USDC adapter. Wallet USDC decreased by **0.005207** from the user's **4.95 USDC** deposit, leaving **4.944793 USDC**, with **0 CELO**. Source: https://forno.celo.org ; transaction explorer above.

## Additional Day 1 integration checks

### Launch setup rechecked 2026-09-09

- Created the agent's facilitator API key through an off-chain ownership signature and stored it privately; configured Railway's `X402_API_KEY`. Production payments are now active and verified. At key creation, the account had 20 mainnet and 1000 testnet settlement credits; that is a historical allocation, not the remaining credit balance. Source: https://x402.celo.org/api/account?address=0x7a1f16230c5F3fD3b50C435fa93035eE789eE789 . Key-creation message and API shape verified in https://x402.celo.org/assets/index-DVsQ-8FQ.js .
- The initial wallet check was zero for CELO, USDC and USDT. The user subsequently funded USDC; see the confirmed post-registration balance above. The user subsequently deposited 4.95 USDT and funded the first AskBots round; see the activation record below. Source: https://forno.celo.org (eth_getBalance and ERC-20 balanceOf).
- AskBots funding configuration currently identifies escrow proxy `0x58166D5422717F3Db0c4c76139dE6FB11D4a09cf` and the already confirmed USDT token. Blockscout verifies the proxy and its `AskBotsEscrow` implementation `0x937FD2d24C6cdaB4B434597d6F67Ae2Bca40B892`. The implementation is not a transaction destination. Sources: https://www.askbots.ai/api/chain ; https://celo.blockscout.com/api/v2/smart-contracts/0x58166D5422717F3Db0c4c76139dE6FB11D4a09cf ; https://celo.blockscout.com/api/v2/smart-contracts/0x937FD2d24C6cdaB4B434597d6F67Ae2Bca40B892 .
- Confirmed ABI: `createProject(bytes32 projectId,uint256 budget)` pulls `budget * (costPerResponse + platformFeePerResponse)` USDT from its caller. Read-only mainnet calls confirm USDT address, `costPerResponse=100000`, `platformFeePerResponse=10000`, and `paused=false`; 10 reviews require exactly 1,100,000 atomic USDT (1.10 USDT). The first approval, deposit and activation are now verified; see the activation record below.
- The user requires AskBots credentials to go only to `https://www.askbots.ai/api/*`. An unauthenticated GET to `/api/bot-profiles/me` there returns 401 directly without redirect. The fetched skill still names the bare host; this conflicts with the user's migration notice, so do not follow the bare-host credential examples. Current website docs also claim USDT gas while published CLI 0.2.0 code checks CELO and omits feeCurrency and attribution suffix; stock execution remains prohibited by the user's transaction policy. Sources: https://www.askbots.ai/skill.md ; https://www.askbots.ai/docs ; https://www.npmjs.com/package/askbots/v/0.2.0 .
...
- Registration JSON uses `type: https://eips.ethereum.org/EIPS/eip-8004#registration-v1`, `services`, `x402Support`, and `registrations`. Server configuration now supplies the confirmed identity ID; pending installations still return empty registrations. Source: https://eips.ethereum.org/EIPS/eip-8004 .
- Railway supplies `X-Real-IP` at its public edge. Trust enabled only for Railway deployment. Source: https://docs.railway.com/networking/public-networking/specs-and-limits . Its volumes mount root-owned. After explicit user approval on 2026-09-08, `RAILWAY_RUN_UID=0` was applied to the dedicated Preflight production service, resolving SQLite volume permissions; deployment and public health/preview checks succeeded. The image retains a non-root default, overridden by Railway at runtime. Source: https://docs.railway.com/volumes .

- Live availability check: Blockscout legacy `/api` began returning HTTP 429 while `/api/v2/addresses/.../transactions`, `/token-transfers`, and `/internal-transactions` remained available. The provider paces reads, retries 429 once, and uses v2 observations with incomplete-history flags on legacy failure. v2 response fields and cursors confirmed by read-only requests on 2026-09-08. Sources: https://celo.blockscout.com/api/v2/addresses/0x23Ca5C88009B94aA554dC37beE88517C92f7c07a/transactions ; https://celo.blockscout.com/api/v2/addresses/0x23Ca5C88009B94aA554dC37beE88517C92f7c07a/token-transfers ; https://celo.blockscout.com/api/v2/addresses/0x23Ca5C88009B94aA554dC37beE88517C92f7c07a/internal-transactions .

### AskBots first round activated 2026-09-09

- Historical activation snapshot at 17:03 UTC: project `k17ck4pafdpv9zts8svazajt6s8e3ncq` was **active**, funded for **10 reviews / 1.10 USDT**, with `paidCount=0` and `responsesReceived=0`. All ten reviews subsequently completed; see [baseline evidence](docs/hackathon/baseline.md). Sources: https://www.askbots.ai/api/projects/k17ck4pafdpv9zts8svazajt6s8e3ncq/fund ; authenticated official CLI `projects:myProjects` query at https://calculating-mockingbird-764.convex.cloud/api/query .
- Official-tagged approval: https://celo.blockscout.com/tx/0x6acea7103559e335185eaff820441e8841d7202fe1325a7704e3edda61a70f28 ; deposit: https://celo.blockscout.com/tx/0x9655f6901bd8a1237a82fe136595e64741cff1ee1b15280790f71f5733919376 . Both successful, USDC adapter feeCurrency, `verifyTx` includes `celo_80fe04c6accd`. Exact allowance/deposit 1,100,000 atomic USDT. Confirmed creator is the registered agent wallet.
- Historical balance immediately after review funding: **3.85 USDT**, **4.936866 USDC**. Total review funding gas cost **0.007927 USDC**. This precedes the controlled API test; it is not a current balance. Second round remains unfunded. Source: https://forno.celo.org (ERC-20 balanceOf and transaction receipts).
- `npm run review:fund` defaults to an unsigned preview. Execution uses explicit CIP-64, gas-unit estimation before token-fee filling, exact deposit approval, 0.10 USDC gas cap per transaction, exclusive records before broadcast and receipt/event reconciliation. Stock AskBots CLI execution remains prohibited. Deployment/ABI reverified at https://celo.blockscout.com/api/v2/smart-contracts/0x58166D5422717F3Db0c4c76139dE6FB11D4a09cf and https://celo.blockscout.com/api/v2/smart-contracts/0x937FD2d24C6cdaB4B434597d6F67Ae2Bca40B892 ; pricing at https://www.askbots.ai/api/chain .
