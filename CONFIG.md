...
The free preview is live at https://preflight-production-9071.up.railway.app (verified 2026-09-08). No assigned tag, identity ID, paid reviews, settlement or funding transfer exists yet. Payments remain disabled. No secrets belong in this file. Signed reports, batch, contract checks, tags endpoint, attestation contract, CLI and MCP are deferred per section 9 (Days 2–4).

## Additional Day 1 integration checks

### Launch setup rechecked 2026-09-09

- Created the agent's facilitator API key through an off-chain ownership signature and stored it privately; configured Railway's `X402_API_KEY`. The service remains payment-disabled pending registration/tag. The account has 20 mainnet and 1000 testnet settlement credits. Source: https://x402.celo.org/api/account?address=0x7a1f16230c5F3fD3b50C435fa93035eE789eE789 . Current key-creation message and API shape verified in https://x402.celo.org/assets/index-DVsQ-8FQ.js .
- Read-only wallet balances: 0 CELO, 0 USDC, 0 USDT. No funds are available for identity registration or review funding. Source: https://forno.celo.org (eth_getBalance and ERC-20 balanceOf).
